...
app.get('/api/users', authenticateToken, requireRole(['admin']), (req, res) => {
  ...
  db.query(countSql, search ? [like, like] : [], (e1, c) => {
    if (e1) return fail(res, 'users.count', e1);
    const total = c?.[0]?.total || 0;
    const params = search ? [like, like, limit, offset] : [limit, offset];
    db.query(dataSql, params, (e2, rows) => {
      if (e2) return fail(res, 'users.list', e2);
...
app.post('/api/users', authenticateToken, requireRole(['admin']), (req, res) => {
  const { username, name, role = 'user', is_active = 1, password } = req.body || {};
  if (!username || !name || !password) {
    return res.status(400).json({ message: 'username/name/password required' });
  }
  const hash = bcrypt.hashSync(password, 10);
  db.query(
    'INSERT INTO users (username, name, role, is_active, password_hash) VALUES (?, ?, ?, ?, ?)',
    [username, name, role, is_active ? 1 : 0, hash],
    (err, r) => {
      if (err) return fail(res, 'users.create', err);
...
app.put('/api/users/:id', authenticateToken, requireRole(['admin']), (req, res) => {
  ...
  db.query(
    ...
    (err) => {
      if (err) return fail(res, 'users.update', err);
      res.json({ success: true });
    }
  );
});

// 비밀번호 초기화 PUT /api/users/:id/password
app.put('/api/users/:id/password', authenticateToken, requireRole(['admin']), (req, res) => {
  const id = parseInt(req.params.id);
  const { password } = req.body || {};
  if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid_id' });
  if (!password) return res.status(400).json({ error: 'password required' });
  const hash = bcrypt.hashSync(password, 10);
  db.query('UPDATE users SET password_hash=? WHERE id=?', [hash, id], (err) => {
    if (err) return fail(res, 'users.reset_password', err);
    res.json({ success: true });
  });
});
...
// 등록 (트랜잭션)
app.post('/api/messages', authenticateToken, requireRole(['admin','user']), (req, res) => {
...
// 수정 (트랜잭션)
app.put('/api/messages/:id', authenticateToken, requireRole(['admin','user']),(req, res) => {
  const id  = parseInt(req.params.id);
  const raw = (req.body?.message ?? req.body?.text ?? '').toString();
  const val = raw.trim();
  if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid_id' });
...
// 삭제 (트랜잭션)
app.delete('/api/messages/:id', authenticateToken, requireRole(['admin','user']), (req, res) => {
  const id = parseInt(req.params.id);
  if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid_id' });
